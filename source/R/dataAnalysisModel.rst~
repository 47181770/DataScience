...
- **回归**
  ...
  回归模型的假设条件很苛刻：结果变量或响应变量要是数值型，并且必须是来自正态分布的随机抽样。（很多情况数据并不满足正态分布），回归有许多特殊的变种，对于回归模型的拟合，R提供了几百个函数，参考https://cran.r-project.org/doc/contrib/Ricci-refcard-regression.pdf
