...
.. tip::
  ...
  数据分析的重点是观察数据，而”数据挖掘“的重点是从数据中发现知识规则，数据分析需要人工建模，本节讲解如何对数值型结果变量和一系列数值型和（或）类别型预测变量之间的关系建模。
...
.
    ...
    为了恰当解释OLS模型系数（即模型可解释），数据必须满足一下统计假设：
...
============= ========================
    ...
    1. AIC输出赤池信息统计量，AIC(Akaike Information Criterion)赤池信息准则是衡量统计模型拟合优良性的一种标准，由日本统计学家赤池弘次在1974年提出。建立在熵的概念上，提供了权衡估计模型复杂度和拟合数据优良性的标准。也就是说逐步回归分析以AIC信息统计量为准则，通过选择最小的AIC信息统计量来达到删除或者增加变量的目的。AIC比较两个模型相对直接，如有4个、10个模型比较，需要用逐步回归法（MASS包中的stepAIC()）和全子集回归（leaps包中的reqsubsets()），大部分多回归模型对比情况后者优于逐步回归法，但是变量多时速度慢。变量自动选择是一种辅助方法，不是直接方法，主题背景知识的理解有助于选择最佳模型。

    2. BIC(Bayesian Information Criterion)贝叶斯信息准则与AIC类似，用于模型选择，AIC和BIC均引入了与模型参数个数相关的惩罚项，BIC的惩罚项比AIC的大，考虑了样本数量，样本数量过多时，可有效防止模型精度过高造成的模型复杂度过高。

    3. QAIC，是为了使用过度离散（缺乏拟合），提出了改进AIC的QAIC。

    4. AIC与BIC是常用的两种模型选择方法。

    5. 回归方程的泛化能力检查方法：交叉验证法、相对权重法（relweights()）。
...
- 建模
    基于对业务数据的理解和数据分析的需要，要将各类数据进行整合和关联，使得数据最终能以可视化的方式呈现，让使用者能够快速、高效的获取到数据中有价值的信息，从而做出准确有效的决策。建模过程中要引入数学公式或模型，用以确定数据实体之间的关联关系。不同的业务逻辑和商业需求需要选择不同的数学公式或模型，一个好的模型需要多次测试和迭代优化来完成。
