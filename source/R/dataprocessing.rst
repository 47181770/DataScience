...
.. code:: r
  ...
  > sfdata <- read.csv('D:/bap/000970.csv')
  > head(sfdata)
...
.. code:: r
  ...
  > library("xlsx")
  > > df <- read.xlsx("D:/bap/allstocks.xls", sheetIndex = 1, encoding = 'UTF-8')
  > head(df)
  #  NA.   ts_code symbol     name area industry list_date
  #1   0 000001.SZ 000001 平安银行 深圳     银行  19910403
  #2   1 000002.SZ 000002    万科A 深圳 全国地产  19910129
  #3   2 000004.SZ 000004 国农科技 深圳 生物制药  19910114
...
.. code:: r
  ...
  > df_text <-  read.table("D:/Books/HKU-BusinessAnalytics/bap/000970t.txt", header=T)
  > head(df_text)
      ts_code ann_date end_date    eps dt_eps total_revenue_ps
  1 000970.SZ 20191026 20190930 0.1470 0.1470           2.7711
  2 000970.SZ 20190817 20190630 0.0940 0.0940           1.7654
  3 000970.SZ 20190427 20190331 0.0415 0.0415           0.8642
  4 000970.SZ 20190321 20181231 0.2330 0.2330           3.9096
  5 000970.SZ 20181026 20180930 0.1760 0.1760           2.8239
  6 000970.SZ 20180818 20180630 0.1054 0.1054           1.8482
  > head(df_text$ts_code)
  [1] 000970.SZ 000970.SZ 000970.SZ 000970.SZ 000970.SZ 000970.SZ
  Levels: 000970.SZ
  > 
...
- 5.5 R处理股票数据
     R语言处理股票时序数据，用到TTR包计算移动平均值（平滑）

.. code:: r

 # 将数据框dataframe的列YYYYMMDD字符串转为YYYYMMDD时间日期列 - OK
 > stock.df.wsf <- read.csv("d:\\bap\\stock.csv")
 > as.Date(paste(stock.df.wsf$trade_date), "%Y%m%d") 
 > stock.df.wsf['trade_date'] <-  as.Date(paste(stock.df.wsf$trade_date), "%Y%m%d")
 > class(stock.df.wsf$trade_date)
 [1] "Date"
 > test['MA3'] <- SMA(test$buy_mle_amount,3)
 Error in SMA(test$buy_mle_amount, 3) : could not find function "SMA"
 > library(TTR)
 Warning message:
 程辑包‘TTR’是用R版本3.6.2 来建造的 
 # 计算连续3天的移动平均值
 > test['MA3'] <- SMA(test$buy_mle_amount,3)
 yy <- subset(test, test$trace_date>'2020-1-1')
 # 利用ggplot2作图
 > p2 <- ggplot(yy, aes(x=yy$trade_date)) +
     + geom_line(aes(y=yy$MA3), size=1, color='red') +
     + geom_line(aes(y=yy$MA5), size=1, color='blue') +
     + geom_line(aes(y=yy$MA10), size=1, color='black') 
