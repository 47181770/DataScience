...
常见问题说明及解决参考如下：
  ...
  1. 数据响应变量与预测变量非线性：使用残差图判断，残差计算使用函数residules(lm.fit)

  2. 误差项自相关（时间序列非常常见）：邻近的残差机会相等，残差图判断

  3. 误差项方差非恒定：残差图判断，如果呈漏斗状，则判断非恒定

  4. 离群点：通过学生化残差计算可以判断，使用rstudent(lm.fit)计算

  5. 高杠杆点：计算杠杆统计量，如果hi > (p +1) / n 则属于高杠杆，使用hatvalues(lm.fit)计算

  6. 共线性：预测变量的相关系数矩阵计算，多重共线性可以使用方差膨胀因子计算。函数：vif(lm.fit)，当VIF值大于5或10是确认存在共线性

  7. 零假设H0：当响应变量与预测变量无关时，统计量F约等于1，当F > 1时，拒绝响应变量与预测变量无关的零假设
